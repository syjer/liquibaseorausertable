...
public class CustomJdbcDatabaseSnapshot extends JdbcDatabaseSnapshot {
    ...
    public CustomJdbcDatabaseSnapshot(DatabaseObject[] examples, Database database, SnapshotControl snapshotControl) throws DatabaseException, InvalidExampleException {
        super(examples, database, snapshotControl);
    }

    public CustomJdbcDatabaseSnapshot(DatabaseObject[] examples, Database database) throws DatabaseException, InvalidExampleException {
    ...
    public class CachingDatabaseMetaData extends JdbcDatabaseSnapshot.CachingDatabaseMetaData {
        ...
        public List<CachedRow> getForeignKeys(final String catalogName, final String schemaName, final String tableName,
            ...
            CachingDatabaseMetaData.ForeignKeysResultSetCache foreignKeysResultSetCache = new CachingDatabaseMetaData.ForeignKeysResultSetCache(database, catalogName, schemaName, tableName, fkName);
            ...
            List<CachedRow> indexes =  getResultSetCache("getIndexInfo").get(new ResultSetCache.UnionResultSetExtractor(database) {
                    ...
                    if (database instanceof OracleDatabase) {
                        ...
                        String sql =
                                "SELECT " +
                                        ...
                                        "JOIN USER_INDEXES i ON i.owner=c.index_owner AND i.index_name = c.index_name and i.table_owner = c.table_owner " +
                                        "LEFT OUTER JOIN USER_ind_expressions e ON e.index_owner=c.index_owner AND e.index_name = c.index_name AND e.column_position = c.column_position   " +
                ...
                boolean shouldBulkSelect(String schemaKey, ResultSetCache resultSetCache) {
                    if (database instanceof OracleDatabase || database instanceof MSSQLDatabase) {
                        return getAllCatalogsStringScratchData() != null || (tableName == null && indexName == null) || super.shouldBulkSelect(schemaKey, resultSetCache);
                    }
                    return false;
                }
            });
        ...
        public List<CachedRow> getColumns(final String catalogName, final String schemaName, final String tableName, final String columnName) throws SQLException, DatabaseException {
            ...
            GetColumnResultSetCache getColumnResultSetCache = new GetColumnResultSetCache(database, catalogName, schemaName, tableName, columnName);
        ...
        public List<CachedRow> getNotNullConst(final String catalogName, final String schemaName,
            ...
            GetNotNullConstraintsResultSetCache getNotNullConstraintsResultSetCache = new GetNotNullConstraintsResultSetCache(database, catalogName, schemaName, tableName);
        ...
        private class GetColumnResultSetCache extends ResultSetCache.SingleResultSetExtractor {
            ...
            protected List<CachedRow> oracleQuery(boolean bulk) throws DatabaseException, SQLException {
                ...
                String sql = "select NULL AS TABLE_CAT, 'NO' as IS_AUTOINCREMENT, cc.COMMENTS AS REMARKS," +
                        "TABLE_NAME, COLUMN_NAME, DATA_TYPE AS DATA_TYPE_NAME, DATA_TYPE_MOD, DATA_TYPE_OWNER, " +
                ...
                sql += " ORDER BY TABLE_NAME, c.COLUMN_ID";
        ...
        private class ForeignKeysResultSetCache extends ResultSetCache.UnionResultSetExtractor {
            ...
            protected String getOracleSql(String jdbcSchemaName) {
                String sql = "SELECT  /*+rule*/" +
                        ...
                        "INNER JOIN user_cons_columns fc " +
                        "ON fc.owner = f.owner " +
            ...
            return getResultSetCache("getPrimaryKeys").get(new ResultSetCache.SingleResultSetExtractor(database) {
                ...
                public List<CachedRow> fastFetchQuery() throws SQLException {
                    ...
                    try {
                        ...
                        if (table == null) {
                            List<CachedRow> tables = getTables(catalogName, schemaName, null);
        ...
        public List<CachedRow> getUniqueConstraints(final String catalogName, final String schemaName, final String tableName) throws DatabaseException {
            return getResultSetCache("getUniqueConstraints").get(new ResultSetCache.SingleResultSetExtractor(database) {
                ...
                public List<CachedRow> fastFetchQuery() throws SQLException, DatabaseException {
                    ...
                    return executeAndExtract(createSql(((AbstractJdbcDatabase) database).getJdbcCatalogName(catalogAndSchema), ((AbstractJdbcDatabase) database).getJdbcSchemaName(catalogAndSchema), tableName), getDatabase(), (database instanceof InformixDatabase));
                }
                ...
                public List<CachedRow> bulkFetchQuery() throws SQLException, DatabaseException {
                    ...
                    return executeAndExtract(createSql(((AbstractJdbcDatabase) database).getJdbcCatalogName(catalogAndSchema), ((AbstractJdbcDatabase) database).getJdbcSchemaName(catalogAndSchema), null), getDatabase());
                }
                ...
                private String createSql(String catalogName, String schemaName, String tableName) throws SQLException {
                    ...
                    if( database instanceof Ingres9Database ) {
                        ...
                        sql = "select uc.owner AS CONSTRAINT_SCHEM, uc.constraint_name, uc.table_name,uc.status,uc.deferrable,uc.deferred,ui.tablespace_name, ui.index_name, uc.VALIDATED as VALIDATED, ac.COLUMN_NAME as COLUMN_NAME " +
                                "from user_constraints uc " +
                                "join user_indexes ui on uc.index_name = ui.index_name and uc.table_name=ui.table_name " +
                                ...
                                "LEFT JOIN user_cons_columns ac ON ac.TABLE_NAME = uc.TABLE_NAME AND ac.CONSTRAINT_NAME = uc.CONSTRAINT_NAME "+
